...
class PresentationController extends Controller
{
    ...
    public function store_presentation(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'data' => 'required|array',
                'data.slides' => 'required|array',
                'data.slides.*.content' => 'required'
            ]);
            ...
            $directory = public_path("teacher/{$teacherId}/presentations");
            if (!file_exists($directory)) {
                mkdir($directory, 0755, true);
            }

            // Generate filename
            $filename = Str::slug($request->title) . '_' . date('Y-m-d_His') . '.json';
            ...
            return response()->json([
                ...
                'filepath' => "teacher/{$teacherId}/presentations/{$filename}"
            ]);
