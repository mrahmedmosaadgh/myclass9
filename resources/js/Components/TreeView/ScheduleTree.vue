<template>
  <div class="schedule-tree">
    <div v-for="classroom in processedClassrooms" :key="classroom.id" class="tree-node">
...
// Process schedules data whether it's an array or object
const processedSchedules = computed(() => {
  if (Array.isArray(props.schedules)) {
    return props.schedules[0] || [];
  }
  return props.schedules?.data || props.schedules || [];
});

// Group schedules by classroom
const processedClassrooms = computed(() => {
  const classroomMap = new Map();

  const schedules = processedSchedules.value;
  if (!schedules) return [];

  (Array.isArray(schedules) ? schedules : [schedules]).forEach(schedule => {
...
const getScheduleInfo = (classroomId, day, period) => {
  ...
  const schedule = (Array.isArray(schedules) ? schedules : [schedules]).find(s =>
