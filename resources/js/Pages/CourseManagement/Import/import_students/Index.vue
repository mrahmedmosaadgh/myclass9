<template>
    ...
    <import-reusable
            ...
            :template-route="route('import_students.import.template')"
            :back-route="route('import_students.import.index')"
            :view-route="route('import_students.import.index')"
            :notes="notes"
...
// URLs
const validateUrl = route('import_students.import.validate');
const importUrl = route('import_students.import.process');

// Required headers for validation
const requiredHeaders = ['number', 'classroom', 'full_name'];

// Notes and sample rows (passed to reusable component)
const notes = [
    'number classroom full_name columns are required',
    'Section column is optional (will use "General" if empty)',
    'Duplicate full_name will be skipped',
...
const fullPreviewColumns = [
    ...
    { name: 'full_name', label: 'full_name', field: 'full_name', align: 'left' },
...
const downloadTemplate = () => {
    window.open(route('import_students.import.template'), '_blank');
    step.value = 2;
};
