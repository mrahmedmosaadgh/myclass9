<template>
    <Head title="Period Details" />
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <q-card flat bordered class="q-mb-md">
                <q-card-section>
                <div class="row items-center justify-between q-mb-md">
                    <div class="text-h5">Period Details</div>
                    <q-btn
                        color="primary"
                        icon="add"
                        label="Add New"
...
const tableColumns = [
    ...
    { name: 'from', label: 'From', field: 'from', sortable: true, align: 'left' },
    { name: 'to', label: 'To', field: 'to', sortable: true, align: 'left' },
    { name: 'main', label: 'Main', field: 'main', sortable: true, align: 'center',
      format: val => val ? '✓' : '✗' },
...
const handleSubmit = (data) => {
    ...
    if (editing.value) {
        useForm(data).put(route('period-details.update', editing.value.id), {
            onSuccess: () => {
                $q.notify({
                    color: 'positive',
            ...
            onError: () => {
                $q.notify({
                    color: 'negative',
                    message: 'Error updating period detail',
                    icon: 'error'
                });
                loading.value = false;
            }
        ...
        useForm(data).post(route('period-details.store'), {
            onSuccess: () => {
                $q.notify({
                    color: 'positive',
            ...
            onError: () => {
                $q.notify({
                    color: 'negative',
                    message: 'Error creating period detail',
                    icon: 'error'
                });
                loading.value = false;
            }
...
const deleteRecord = (record) => {
    $q.dialog({
        title: 'Confirm',
        message: 'Are you sure you want to delete this period detail?',
        cancel: true,
        persistent: true
    }).onOk(() => {
        loading.value = true;
        useForm().delete(route('period-details.destroy', record.id), {
            ...
            onError: () => {
                $q.notify({
                    color: 'negative',
                    message: 'Error deleting period detail',
                    icon: 'error'
                });
                loading.value = false;
            }
...
const goToPage = (page) => {
    if (page) {
        window.location.href = route('period-details.index', { page });
    }
};
</script>