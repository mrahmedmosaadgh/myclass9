...
// Computed property to ensure colors are always available
const gradientColors = computed(() => {
  return {
    start: props.options?.colors?.start || '#6366f1',
    end: props.options?.colors?.end || '#4f46e5'
  };
});
...
<template>
  <div
    ...
    :style="{
      ...
      height: `${options?.height || 380}px`
    }"
  >
    <!-- Book spine -->
    <div class="book-spine">
      <div class="spine-text">{{ options?.title || 'Title' }}</div>
    </div>
    ...
    <div
      ...
      :style="{
        background: `linear-gradient(145deg, ${gradientColors.start}, ${gradientColors.end})`
      }"
    >
      <div class="cover-inner">
        <div class="cover-content">
          <slot name="cover">
            <h2 class="cover-title">{{ options?.title || 'Title' }}</h2>
            <div class="cover-subtitle">{{ options?.subtitle || 'Subtitle' }}</div>
          </slot>
        </div>
      </div>
    </div>
