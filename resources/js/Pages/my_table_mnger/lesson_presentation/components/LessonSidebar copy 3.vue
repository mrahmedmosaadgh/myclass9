<template>
    <div>
        <!-- Custom Sidebar -->
        <div
            class=" max-[1200px]:fixed  inset-y-0 left-0 z-40 w-72 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out min-[1200px]:translate-x-0 min-[1200px]:static min-[1200px]:inset-auto min-[1200px]:z-auto flex flex-col h-full shadow-2xl min-[1200px]:shadow-none"
            :class="
                showDrawer
                    ? 'translate-x-0'
                ...
                <q-list separator>
                    ...
                    >
                        ...
                        <q-item
                            clickable
                            v-ripple
                            @click="$emit('update:currentSection', section.id);$emit('update:currentSection_data', section)"
                        ...
                        >
                            <q-item-section avatar class="min-w-0 pr-3">
                                ...
                                >
                                    <q-icon
                                        :name="section.qIcon || section.icon"
                                        size="16px"
                                    />
                                </div>
                            </q-item-section>
                            <q-item-section>
                                ...
                                >
                                    {{
                                        getSectionSlideCount(section.id)
                                    }}
                                    slide(s)
                                </q-item-label>
                            </q-item-section>
                        ...
                        <q-slide-transition>
                            <div
                                ...
                                class="bg-gray-50/50 border-t border-gray-100/50 p-3 pl-4"
                            >
                                ...
                                >
                                    <div
                                        class="bg-gray-100 group-hover:bg-blue-100 rounded-full p-0.5 transition-colors"
                                    >
                                ...
                                <!-- Slides -->
                                <div
                                    class="space-y-2 pl-1 border-l-2 border-gray-200 ml-1.5"
                                >
                                    <div
                                        v-for="(
                                            slide, index
                                        ) in getSectionSlides(section.id)"
                                        :key="slide.id || index"
                                        @click.stop="
                                        ...
                                        :class="
                                            isSlideActive(slide)
                                                ? 'bg-white border-blue-500 shadow-sm ring-1 ring-blue-500/20 z-10'
                                    ...
                                    >
                                        ...
                                        <div
                                            class="text-[11px] text-gray-500 truncate pl-2 border-l-2"
                                            :class="
                                                isSlideActive(slide)
                                                    ? 'border-blue-200'
                                                    : 'border-gray-100'
                                    ...
                                    <div
                                        v-if="
                                            getSectionSlides(section.id)
                                                .length === 0
                                        "
                                        class="ml-2 text-center py-6 text-gray-400 text-xs italic bg-gray-50 rounded-lg border border-dashed border-gray-200"
                                    >
                                        {{
                                            canEdit
                                                ? "No slides yet"
        ...
        <!-- Overlay for mobile - only show when drawer is open AND on mobile -->
        <!-- <transition name="fade">
            <div
                v-if="showDrawer"
                class="fixed inset-0 bg-black mouse-events-none  opacity-10 z-30 min-[1200px]:hidden"
                @click="$emit('update:showDrawer', false)"
            ></div>
        </transition> -->

        <!-- Mobile: Floating Toggle Button - only show when drawer is closed -->
        <transition name="slide-up">
            <q-btn
                ...
                color="primary"
                icon="menu"
                class="fixed bottom-6 left-6 z-50 min-[1200px]:hidden shadow-lg"
