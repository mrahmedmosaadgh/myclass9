    ...
    <div class="space-y-3">
      ...
      <div v-if="question.type === 'single_choice'" class="space-y-2">
        <button 
          ...
          class="w-full text-left p-4 rounded-lg border transition-all flex items-center justify-between group"
      ...
      <div v-if="question.type === 'multiple_choice'" class="space-y-2">
        <button 
          ...
          class="w-full text-left p-4 rounded-lg border transition-all flex items-center gap-3"
          :class="getOptionCheckboxClass(option.id)"
        >
      ...
      <div v-if="question.type === 'true_false'" class="flex gap-4">
        <button 
          ...
          class="flex-1 p-4 rounded-lg border text-center font-medium transition-all"
      ...
      <!-- Short Answer / Number -->
      <div v-if="['short_answer', 'number'].includes(question.type)" class="space-y-2">
        <input 
          type="text" 
          :value="answer || ''"
          @input="!submitted && updateAnswer($event.target.value)"
          class="w-full p-4 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
          :disabled="submitted"
      ...
      <!-- Long Answer -->
      <div v-if="question.type === 'long_answer'" class="space-y-2">
        <textarea 
          :value="answer || ''"
          @input="!submitted && updateAnswer($event.target.value)"
          class="w-full p-4 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all min-h-[120px]"
          :disabled="submitted"
          placeholder="Type your answer here..."
        ></textarea>
      </div>
...
const checkCorrectness = () => {
  ...
  if (q.type === 'single_choice') {
    ...
    return correctOption && a === correctOption.id;
  } else if (q.type === 'multiple_choice') {
    const correctOptions = q.options.filter(o => o.is_correct).map(o => o.id);
    if (!Array.isArray(a) || a.length !== correctOptions.length) return false;
    return a.every(val => correctOptions.includes(val));
  } else if (q.type === 'true_false') {
    return a === (q.correct_answer === true || q.correct_answer === 'true' || q.correct_answer === 1);
  } else if (['short_answer', 'number'].includes(q.type)) {
...
const correctAnswerText = computed(() => {
  const q = props.question;
  if (q.type === 'single_choice') return q.options.find(o => o.is_correct)?.text || 'Unknown';
...
const submit = () => {
  ...
  if (checkCorrectness()) {
    ...
    emit('update:solved', true); // Failed but done
    if (timerInterval) clearInterval(timerInterval);
  }
};
...
const getOptionClass = (optionId) => {
  ...
  const isCorrectOption = props.question.options.find(o => o.id === optionId)?.is_correct;
  if (isCorrectOption) return 'bg-green-50 border-green-500 ring-1 ring-green-500';
  if (selected && !isCorrectOption) return 'bg-red-50 border-red-500 ring-1 ring-red-500';
  return 'bg-white border-gray-200 opacity-50';
};

const getOptionCheckboxClass = (optionId) => getOptionClass(optionId);
...
const getTrueFalseClass = (val) => {
  ...
  const isCorrectValue = (props.question.correct_answer === true || props.question.correct_answer === 'true' || props.question.correct_answer === 1) === val;
  if (isCorrectValue) return 'bg-green-600 text-white border-green-600';
  if (selected && !isCorrectValue) return 'bg-red-600 text-white border-red-600';
